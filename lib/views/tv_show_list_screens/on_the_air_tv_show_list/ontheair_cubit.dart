...
part 'ontheair_state.dart';

class OnTheAirTVShowCubit extends BaseTVShowListCubit<OnTheAirTVShowState> {
  OnTheAirTVShowCubit({
    BaseTVShowRepository? tvShowRepo,
  })  : _tvShowRepo = tvShowRepo ?? GetIt.I<BaseTVShowRepository>(),
        super(OnTheAirTVShowState.init());
  ...
  Future<void> loadTVShows({bool more = false}) async {
    ...
    try {
      ...
      final tvShows = await _tvShowRepo.getOnTheAir(page: nextPage);
      ...
      ErrorHandler.catchIt(
        error: e,
        stackTrace: st,
        customUnknownErrorMessage: 'Failed to fetch on the air TV shows. Please try again.',
        onCatch: _catchError,
      );
    }
  }
